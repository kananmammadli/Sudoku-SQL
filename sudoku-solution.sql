...
res as (
	...
	model
		...
		rules AUTOMATIC ORDER ( 
			val[any,any] = case when val[cv(rr), cv(cc)] in (1,2,4,8,16,32,64,128,256) 
									then val[cv(rr), cv(cc)]
								else bitand(
										bitand(
											bitand(
												exclval(cv(cc), 
														line_type(val[cv(rr),1],val[cv(rr),2],val[cv(rr),3],val[cv(rr),4],val[cv(rr),5],val[cv(rr),6],val[cv(rr),7],val[cv(rr),8],val[cv(rr),9])),
												exclval(cv(rr), 
														line_type(val[1,cv(cc)],val[2,cv(cc)],val[3,cv(cc)],val[4,cv(cc)],val[5,cv(cc)],val[6,cv(cc)],val[7,cv(cc)],val[8,cv(cc)],val[9,cv(cc)]))
											),
											exclval(mod(cv(rr)-1,3)*3+mod(cv(cc)-1,3)+1 /* position number in 3*3 section*/, 
													line_type(	val[ceil(cv(rr)/3)*3-2, ceil(cv(cc)/3)*3-2], val[ceil(cv(rr)/3)*3-2, ceil(cv(cc)/3)*3-1], val[ceil(cv(rr)/3)*3-2, ceil(cv(cc)/3)*3-0],
										...
										bitand(
											...
											exclset(mod(cv(rr)-1,3)*3+mod(cv(cc)-1,3)+1 /* position number in 3*3 section*/, 
													line_type(	val[ceil(cv(rr)/3)*3-2, ceil(cv(cc)/3)*3-2], val[ceil(cv(rr)/3)*3-2, ceil(cv(cc)/3)*3-1], val[ceil(cv(rr)/3)*3-2, ceil(cv(cc)/3)*3-0],
																val[ceil(cv(rr)/3)*3-1, ceil(cv(cc)/3)*3-2], val[ceil(cv(rr)/3)*3-1, ceil(cv(cc)/3)*3-1], val[ceil(cv(rr)/3)*3-1, ceil(cv(cc)/3)*3-0],
																val[ceil(cv(rr)/3)*3-0, ceil(cv(cc)/3)*3-2], val[ceil(cv(rr)/3)*3-0, ceil(cv(cc)/3)*3-1], val[ceil(cv(rr)/3)*3-0, ceil(cv(cc)/3)*3-0])
											)
